...
mutable struct Spectral_Spherical_Mesh
    ...
    # Dimensions and resolution parameters
    num_fourier::Int64                 # maximum nubmer of zonal waves
    num_spherical::Int64               # maximum number of spherical harmonics
    nλ::Int64                          # zonal grids
    nθ::Int64                          # meridional grids
    nd::Int64                          # vertical grids
    radius::Float64                    # radius of the planet
    

    
    # Physical grid geometry
    sinθ::Array{Float64,1}             # sine of latitude
    cosθ::Array{Float64,1}             # cosine of latitude
    wts::Array{Float64,1}              # Gaussian quadrature weights
    λc::Array{Float64,1}               # longitude cell center
    θc::Array{Float64,1}               # latitude cell center
    λe::Array{Float64,1}               # longitude cell boundary 
    θe::Array{Float64,1}               # latitude cell boundary 
    


    # Spectral basis functions
    qnm::Array{Float64,3}              # associated legendre polynomials
    dqnm::Array{Float64,3}             # derivatives of legendre polynomials
    qwg::Array{Float64,3}              # weighted legendre polynomials
    epsilon::Array{Float64,2}          # recurrence normalization coefficients
    


    # Differential operators
    coef_alp_a::Array{Float64,3}       # zonal alpha operators
    coef_alp_b::Array{Float64,3}       # meridional alpha operators

    coef_uvm::Array{Float64,2}         # uv coefficients (minus)
    coef_uvc::Array{Float64,2}         # uv coefficients center
    coef_uvp::Array{Float64,2}         # uv coefficients (plus)
    
    coef_dλ::Array{Float64,2}          # zonal gradient coefficients
    coef_dθm::Array{Float64,2}         # meridional gradient coefficients (minus)
    coef_dθp::Array{Float64,2}         # meridional gradient coefficients (plus)
    
    laplacian_eig::Array{Float64,2}    # Laplacian eigenvalues
    wave_numbers::Array{Int64, 2}      # total wavenumbers
...
function Trans_Spherical_To_Grid!(mesh::Spectral_Spherical_Mesh, snm::Array{ComplexF64,3},  pfield::Array{Float64,3})
    """
    Performs the Inverse Spectral Transform (Synthesis) to convert spectral coefficients 
    back to physical grid values.
    
    Parameters
    ----------
    mesh (Spectral_Spherical_Mesh)
        The mesh structure containing resolution parameters (num_fourier, nλ, nθ), 
        pre-computed Legendre polynomials (qnm), and workspace buffers.

    snm (num_fourier+1, num_spherical+1, nd)
        Spectral coefficients F_{m,n} (Spherical Harmonic modes).
        Complex-valued array representing the field in spectral space.
        The third dimension `nd` is the number of vertical levels (can be 1).

    Returns
    -------
    None
        (The result is stored in-place in `pfield`)

    Modified
    --------
    pfield (nλ, nθ, nd)
        Physical field values F(λ, θ) on the Gaussian grid.
        Overwritten with the result of the synthesis (Spectral -> Fourier -> Grid).
    
    mesh (Spectral_Spherical_Mesh)
        Internal workspace buffers (fourier_d1/d2 or fourier_ds1/ds2) are modified 
        during the intermediate Fourier transform steps.
...
function Trans_Grid_To_Spherical!(mesh::Spectral_Spherical_Mesh, pfield::Array{Float64,3}, snm::Array{ComplexF64,3})
    """
    Performs the Forward Spectral Transform (Analysis) to project physical grid 
    values onto spherical harmonic modes.
    
    Parameters
    ----------
    mesh (Spectral_Spherical_Mesh)
        The mesh structure containing resolution parameters, pre-computed 
        Legendre weights (qwg), and workspace buffers.
        
    pfield (nλ, nθ, nd)
        Physical field values F(λ, θ) on the Gaussian grid.

    snm (num_fourier+1, num_spherical+1, nd)
        Buffer to store the resulting spectral coefficients F_{m,n}.

    Returns
    -------
    None
        (The result is stored in-place in `snm`)

    Modified
    --------
    snm (Array{ComplexF64,3})
        Overwritten with the spectral coefficients F_{m,n}.
        The highest spherical mode (num_spherical+1) is explicitly zeroed out 
        to ensure consistent triangular truncation.

    mesh (Spectral_Spherical_Mesh)
        Internal workspace buffers (fourier_d1 or fourier_ds1) are modified 
        during the intermediate Fourier transform steps.
...
function Trans_Grid_To_Fourier!(mesh::Spectral_Spherical_Mesh, pfield::Array{Float64,3}, fourier_g::Array{ComplexF64,3})
    ...
    decomposing the data into zonal wavenumbers without performing the meridional
    Legendre transform.

    Parameters
    ----------
    mesh (Spectral_Spherical_Mesh)
        The mesh structure providing grid dimensions (nλ, nθ).

    pfield (nλ, nθ, nd)
        Physical field values F(λ, θ) on the Gaussian grid.

    fourier_g (nλ, nθ, nd)
        Buffer to store the intermediate Fourier coefficients g_m(θ).

    Returns
    -------
    None
        (The result is stored in-place in `fourier_g`)

    Modified
    --------
    fourier_g
        Overwritten with the normalized Fourier coefficients.
        Computed as FFT(pfield) / nλ.
...
function Compute_Alpha_Operator_Init(num_fourier::Int64, num_spherical::Int64, 
                                     ...
                                     wts::Array{Float64,1}) 
    """
    See Compute_Alpha_Operator! for usage.
